...
test('Studio deep links, history, pending recovery and server chapter identity', async ({ page }) => {
  ...
  await page.waitForURL(`**/studio/${second}?view=Create`);
  if (writes !== 1) throw new Error(`Expected one flushed write, got ${writes}`);
  await page.goBack();
  await page.waitForURL(`**/studio/${first}?view=Create`);
  ...
  await page.getByRole('button', { name: '上一个页面', exact: true }).click();
  await page.waitForURL('**view=Detail');
  await page.reload();
  await page.getByRole('region', { name: 'Detail 工作区', exact: true }).waitFor();
  if (await page.getByRole('complementary', { name: '章节侧边栏' }).count()) throw new Error('Detail sidebar leak');
  await page.getByRole('button', { name: '下一个页面', exact: true }).click();
  await page.waitForTimeout(650);
  await page.getByRole('button', { name: '展开章节侧边栏', exact: true }).click();
  await page.locator('.studio-directory summary').first().hover();
  await page.getByRole('button', { name: '在第一卷新建章节', exact: true }).click();
  await page.waitForURL(`**/studio/${createdId}?view=Create`);
