...
def test_mvp_models_are_imported_and_registered() -> None:
    ...
    assert ReaderPanelInvocation.__table__.c.work_key.nullable is False
    configure_mappers()
...
def test_reader_panel_model_constraint_and_index_names_match_migration_contract() -> None:
    session_constraints = {
        c.name
        for c in ReaderPanelSession.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    run_constraints = {
        c.name
        for c in ReaderRun.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    report_constraints = {
        c.name
        for c in ReaderInitialReport.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    issue_constraints = {
        c.name
        for c in ReaderPanelIssue.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    ballot_constraints = {
        c.name
        for c in ReaderPanelBallot.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    message_constraints = {
        c.name
        for c in ReaderPanelMessage.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    invocation_constraints = {
        c.name
        for c in ReaderPanelInvocation.__table__.constraints
        if isinstance(c, (CheckConstraint, UniqueConstraint))
    }
    ...
    assert "uq_reader_panel_invocations_work_attempt" in invocation_constraints
    assert "ck_reader_panel_invocations_status" in invocation_constraints
    assert "ck_reader_panel_invocations_safe_error" in invocation_constraints
    assert "ck_reader_panel_invocations_work_key" in invocation_constraints
